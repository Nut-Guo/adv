...
class PatchNet(pl.LightningModule):
    def __init__(self, yolo_version, patch_size=100, init_patch='random', alpha=0.1, log_interval=100, patch_transforms=None, *args, **kwargs) -> None:
        super().__init__()
    ...
    def step(self, batch: Any, batch_idx: int):
        ...
        if batch_idx % self.log_interval == 0:
            self.logger.experiment.log({
                'patch': wandb.Image(adv_batch[0].clone().detach().permute((1, 2, 0))),
                'patched_img': wandb.Image(image_batch[0].clone().detach().permute((1, 2, 0)))
            })
