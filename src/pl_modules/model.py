...
class PatchNet(pl.LightningModule):
    ...
    def step(self, batch: Any, batch_idx: int):
        ...
        self.patch.data = self.patch.data.clamp(0, 1)
        ...
        tv_loss = tv * 0.1
        det_loss = -torch.log(1 - torch.mean(torch.cat(pred))) if len(pred) > 0 else torch.tensor(0.1)
        self.log_dict(
            {
                'det_loss': det_loss,
                'tv_loss': tv_loss,
        ...
        patch = wandb.Image(self.patch.clone().detach().cpu())
